...
class HWrec_1:
 def __init__(self,metaline,headpart,matches):
...
def get_hwrecs1(groups):
 ...
 for ientry,e in enumerate(entries):
  ...
  a = '\n'.join(group[2:-1])
  groupline = '%s\n%s' %(tailpart,a)
  subhws = re.findall(r'Ⓝ[^➔]*➔',groupline,re.DOTALL)
  hwrec = HWrec_1(metaline,headpart,subhws)
  hwrecs.append(hwrec)
 return hwrecs

def write_1(fileout,recs):
 outarr = []
 nmax = 0
...
if __name__=="__main__":
 ...
 if option == '1':
  hwrecs= get_hwrecs1(groups)
  write_1(fileout,hwrecs)

 else:
  print('ERROR. Unknown option',oprion)
  exit(1)
